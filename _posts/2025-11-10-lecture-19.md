...
- **CNN**: Captures local patterns with fixed-size kernels, but cannot directly handle variable-length input, so sequences must be padded to a fixed length.
...
- The gradient w.r.t. earlier time steps involves a product of many Jacobians across time, which can make its norm shrink to (almost) zero (vanishing gradients) or grow very large (exploding gradients).
